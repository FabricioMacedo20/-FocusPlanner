...
<div class="max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold mb-6">Criar meta</h1>
    ...
    <div class="bg-slate-800 rounded-xl p-6 shadow">
        <form method="POST" action="{{ route('goals.store') }}">
            ...
            <div class="mb-4">
                ...
                <input name="title" value="{{ old('title') }}" class="w-full mt-2 p-2 rounded bg-slate-900 border border-slate-700" placeholder="Ex: Aprender Laravel" />
                @error('title')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="target_value" value="{{ old('target_value', 100) }}" class="w-full mt-2 p-2 rounded bg-slate-900 border border-slate-700" />
                    @error('target_value')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="current_value" value="{{ old('current_value', 0) }}" class="w-full mt-2 p-2 rounded bg-slate-900 border border-slate-700" />
                    @error('current_value')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
